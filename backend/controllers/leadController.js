...
const { detectDeviceType, getQualificationTier, normalizeWhitespace } = require('../utils/leadUtils');
...
const normalizeLeadPayload = (body, userAgent) => {
  ...
  const sebiRegistered =
    ...
    body.sebiRegistered === 'yes'
      ? true
      : body.sebiRegistered === false || body.sebiRegistered === 'false'
        ? false
        : null;
...
const submitConsultationLead = async (req, res, next) => {
  try {
    const leadPayload = normalizeLeadPayload(req.body, req.headers['user-agent']);
    ...
    const lead = await ConsultationLead.create({
      ...
      integrationStatus: {
        notion:
          process.env.NOTION_API_KEY || process.env.NOTION_WEBHOOK_URL ? 'pending' : 'disabled',
        googleSheets: process.env.GOOGLE_SHEETS_WEBHOOK_URL ? 'pending' : 'disabled',
        crm: process.env.CRM_WEBHOOK_URL ? 'pending' : 'disabled',
      },
      emailStatus: {
        user: process.env.RESEND_API_KEY ? 'pending' : 'disabled',
        owner:
          process.env.RESEND_API_KEY && process.env.INTERNAL_NOTIFICATION_EMAIL
            ? 'pending'
            : 'disabled',
      },
    });
...
const listConsultationLeads = async (req, res, next) => {
  try {
    const leads = await ConsultationLead.find().sort({ submittedAt: -1 }).limit(200);
