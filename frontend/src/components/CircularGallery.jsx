...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  ...
  if (!context) {
    return { texture: new Texture(gl, { generateMipmaps: false }), width: 128, height: 40 }
  }
  context.font = font
...
class Media {
  ...
  update(scroll, direction) {
    ...
    if (this.bend === 0) {
      ...
      const absBend = Math.abs(this.bend)
      const radius = (halfViewportWidth * halfViewportWidth + absBend * absBend) / (2 * absBend)
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    if (!isWebglSupported) return undefined

    let app
    try {
      app = new OglGallery(containerRef.current, {
        items,
        bend,
        textColor,
        borderRadius,
      ...
      console.error('CircularGallery failed to initialize:', error)
  ...
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, isWebglSupported])
  ...
  if (hasError || !isWebglSupported) {
    return (
      <div className="flex h-full w-full items-center justify-center rounded-xl border border-white/10 bg-black/30 text-sm text-white/80">
        Gallery unavailable on this device/browser.
      </div>
    )
  }
