...
const Form = ({
  ...
  const form = useAppStore((state) => state.forms[formId] || state.forms.default);
    ...
    <section className="relative overflow-hidden rounded-4xl border border-white/20 bg-[#111317] p-4 font-body text-white shadow-[0_20px_60px_rgba(0,0,0,0.45)] sm:p-5">
      ...
      <div className="relative">
        ...
        <form className="space-y-3.5" onSubmit={handleSubmit}>
          <div>
            <label htmlFor="name" className="mb-1.5 block font-body text-sm font-medium text-gray-100">
              Name
            </label>
            <input
              id="name"
            ...
            />
            <p className="mt-1.5 font-body text-xs text-gray-400">How you&apos;d like to be addressed.</p>
          </div>

          <div>
            <label htmlFor="email" className="mb-1.5 block font-body text-sm font-medium text-gray-100">
              Email
            </label>
            <input
              id="email"
              ...
              className="w-full rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm text-white placeholder:text-gray-400 outline-none transition duration-300 focus:-translate-y-0.5 focus:border-white/40 focus:bg-white/10 focus:shadow-[0_0_0_3px_rgba(255,255,255,0.12)]"
            />
            <p className="mt-1.5 font-body text-xs text-gray-400">Where I can reach you.</p>
          </div>

          <div>
            <label htmlFor="phone" className="mb-1.5 block font-body text-sm font-medium text-gray-100">
              Phone (optional)
            </label>
            <input
              id="phone"
              ...
              className="w-full rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm text-white placeholder:text-gray-400 outline-none transition duration-300 focus:-translate-y-0.5 focus:border-white/40 focus:bg-white/10 focus:shadow-[0_0_0_3px_rgba(255,255,255,0.12)]"
            />
            <p className="mt-1.5 font-body text-xs text-gray-400">Only if you prefer a quick call.</p>
          </div>

          <div>
            <label htmlFor="category" className="mb-1.5 block font-body text-sm font-medium text-gray-100">
              Service Interest
            </label>
            <select
              id="category"
          ...
          {form.error ? <p className="text-center text-xs text-red-300">{form.error}</p> : null}
          {form.successMessage ? (
            <p className="text-center text-xs text-emerald-300">{form.successMessage}</p>
          ) : null}
